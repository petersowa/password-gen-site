...
export class AppHashPW extends Component {
  ...
  makePassword = (term) => {
    ...
    crypto.subtle.digest('SHA-256', new TextEncoder('utf-8').encode(term)).then(val => {
      const pw = []
      ...
      <div>
        ...
        <Box bg="transparent" {...Box.float("100%", "100%", "none", "auto") }>
          ...
          <BoxNav bg="grey" fg="lightgrey" { ...Box.style({ margin: 0, padding: 10, borderStyle: "none", float: "right" }) }>
            <input placeholder='' value={this.state.term} onChange={this.updateTerm} />
        ...
        {(this.state.password.length > 0) &&
          <Box bg="#fff" {...Box.float("24rem", "100%", "none", "10px auto") } >
            <Box bg="silver"
